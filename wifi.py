...
def set_access_point_mode():
    ...
    result = subprocess.run(['sudo', 'nmcli', 'device', 'wifi', 'hotspot', 'ifname', 'wlan0', 'ssid', 'sandalkesehatan', 'password', '12345678'], capture_output=True, text=True)    
    ...
    # Step 5: Configure dnsmasq to handle DHCP in the range 192.168.4.10-192.168.4.20 without a gateway
    with open('/etc/dnsmasq.conf', 'w') as f:
        f.write("interface=wlan0\n")
        f.write("dhcp-range=192.168.4.10,192.168.4.20,255.255.255.0,24h\n")
        f.write("dhcp-option=3\n")  # This disables the default gateway
    ...
    print("Switched to Access Point mode (local network only, DHCP pool set).")
